...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/29 23:06:50 by yhadhadi          #+#    #+#             */
/*   Updated: 2024/05/12 06:36:02 by yhadhadi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (true)
	{
		if (strm.root_blk && strm.root_blk->len == 0)
			break ;
		eol = seek_eol(&strm);
		if (eol)
		{
			line = retrieve_line(&strm, eol);
			if (!line)
				break ;
			return (line);
		}
		if (!read_data_blk(&strm, fd))
			break ;
	}
	return (clear_data_strm(&strm), NULL);
}
